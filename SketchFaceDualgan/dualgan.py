...
class DUALGAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # Load the datasets
        (X_A, _) = dataset.load("sketches")
        (X_B, _) = dataset.load("pictures")

        # Rescale -1 to 1
        X_A = (X_A.astype(np.float32) - 127.5) / 127.5
        X_B = (X_B.astype(np.float32) - 127.5) / 127.5
    ...
    def save_imgs(self, epoch, X_A, X_B):
        ...
        # Sample generator inputs
        imgs_A = self.sample_generator_input(X_A, sample_size)
        imgs_B = self.sample_generator_input(X_B, sample_size)
        ...
        # Reshape images
        imgs_A = imgs_A.reshape((sample_size, self.img_rows, self.img_cols, 1))
        imgs_B = imgs_B.reshape((sample_size, self.img_rows, self.img_cols, 1))
        fake_A = fake_A.reshape((sample_size, self.img_rows, self.img_cols, 1))
        fake_B = fake_B.reshape((sample_size, self.img_rows, self.img_cols, 1))

        if not os.path.exists('dualgan_results'):
            os.makedirs('dualgan_results')

        if not os.path.exists('dualgan_results/sketch2face_input'):
            os.makedirs('dualgan_results/sketch2face_input')
        if not os.path.exists('dualgan_results/sketch2face_input/' + str(epoch)):
            os.makedirs('dualgan_results/sketch2face_input/' + str(epoch))

        if not os.path.exists('dualgan_results/sketch2face_output'):
            os.makedirs('dualgan_results/sketch2face_output')
        ...
        if not os.path.exists('dualgan_results/face2sketch_input'):
            os.makedirs('dualgan_results/face2sketch_input')
        if not os.path.exists('dualgan_results/face2sketch_input/' + str(epoch)):
            os.makedirs('dualgan_results/face2sketch_input/' + str(epoch))

        if not os.path.exists('dualgan_results/face2sketch_output'):
            os.makedirs('dualgan_results/face2sketch_output')
